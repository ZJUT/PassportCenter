...
public class BachelorIDResolver extends IDResolver
{
    ...
    /**
     * this is a entrance for polymorphic implementation of doResolve for Bachelor degree<BR/>
     ...
     *
     * with the help of data above, we can doResolve all information concern about this bachelor student
     *
    ...
    public ResolvedID doResolve(Passport passport)
    {
        ...
        id.put("school", getSchoolService().findById(sid));

        String mid = passport.getId().substring(4, 8);
        id.put("major", getMajorService().findById(mid));
